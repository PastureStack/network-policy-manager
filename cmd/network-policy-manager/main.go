...
func runServe(args []string, stdout, stderr io.Writer) int {
	...
	cleanupOnExit := flags.Bool("cleanup-on-exit", false, "remove the exact owned firewall table after a graceful shutdown")
	if err := flags.Parse(args); err != nil {
		return 2
	}
	if flags.NArg() != 0 {
	...
	if !managerStopped {
		...
		case <-time.After(5 * time.Second):
			fmt.Fprintln(stderr, "policy manager did not stop cleanly")
			return 1
		}
	}
	...
	if *cleanupOnExit {
		...
		defer cleanupCancel()
		if err := backend.Cleanup(cleanupContext); err != nil {
			fmt.Fprintln(stderr, "unable to remove owned firewall state during shutdown")
			return 1
		}
		fmt.Fprintln(stdout, "network policy manager stopped; exact owned host rules were removed")
		return 0
	}
