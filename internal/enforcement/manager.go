...
func (manager *Manager) reconcile(parent context.Context) {
	...
	if err != nil {
		manager.recordFailure(now, "metadata", err)
		return
	}
	plan, err := Compile(snapshot)
	if err != nil {
		manager.recordFailure(now, "compile", err)
		return
	}
	if plan.Digest != manager.appliedHash {
		if err := manager.Backend.Apply(ctx, plan); err != nil {
			manager.recordFailure(now, "firewall", err)
			return
		}
...
func (manager *Manager) recordFailure(now time.Time, stage string, cause error) {
	failures := manager.failures.Add(1)
	manager.mu.Lock()
	shouldLog := manager.lastFailureLog.IsZero() ||
		now.Sub(manager.lastFailureLog) >= 30*time.Second
	if shouldLog {
		manager.lastFailureLog = now
	}
	...
	manager.mu.Unlock()
	if shouldLog && manager.Logger != nil {
		manager.Logger.Printf("reconcile degraded stage=%s reason=%v", stage, cause)
	}
	...
	manager.status.DefaultAction = plan.DefaultAction
